...
/**
 * 当前操作的元信息记录， 主要包含数据操作前后的文件变更记录， 用户更新到 manifest 文件中。
 * */
public class CommitMessageImpl implements CommitMessage {
    ...
    private transient BinaryRow partition;                  // 分区信息
    private transient int bucket;                           // 桶信息
    private transient DataIncrement dataIncrement;          // 数据增量信息, 包含新增文件，删除文件，与变更文件
    private transient CompactIncrement compactIncrement;    // 压缩增量信息, 包含压缩之前数据， 压缩之后数据
    private transient IndexIncrement indexIncrement;        // 新的索引文件记录
