...
trait ExpressionHelper extends PredicateHelper {
  ...
  def resolveFilter(spark: SparkSession, plan: LogicalPlan, conditionSql: String): Expression = {
    val unResolvedExpression = spark.sessionState.sqlParser.parseExpression(conditionSql)  // 解析过滤表达式
    val filter = Filter(unResolvedExpression, plan)                                        // 生成过滤节点
    spark.sessionState.analyzer.execute(filter) match {                                    // 生成分析条件树
