...
public class CompactProcedure extends BaseProcedure {
    ...
    public InternalRow[] call(InternalRow args) {
        ...
        List<String> sortColumns = blank(args, 3) ? Collections.emptyList() : Arrays.asList(args.getString(3).split(","));
        ...
        return modifyPaimonTable(
                ...
                table -> {
                    ...
                    if (!StringUtils.isBlank(finalWhere)) {
                        ...
                        checkArgument(ExpressionUtils.isValidPredicate(
                                        spark(),
                                        condition,
                    ...
                    InternalRow internalRow = newInternalRow(execute((FileStoreTable) table, sortType, sortColumns, relation, condition));
    ...
    private boolean execute(
            FileStoreTable table,              // 目标表
            String sortType,                   // 排序策略
            List<String> sortColumns,          // 排序列
            LogicalPlan relation,              // 表对应的计划树
            @Nullable Expression condition) {  // 分区过滤条件
    ...
    private void compactAwareBucketTable(
        ...
        // TODO-1 : 从 snapshot 中获取到要压缩的分区，桶信息
        SnapshotReader snapshotReader = table.newSnapshotReader();   // Snapshot Reader

        if (filter != null) {
            snapshotReader.withFilter(filter);
        }

        // 获取要压缩的(partition -> bucket 映射关系)
        List<Pair<byte[], Integer>> partitionBuckets = snapshotReader.read().splits().stream()
            .map(split -> (DataSplit) split)
            .map(dataSplit -> Pair.of(dataSplit.partition(), dataSplit.bucket()))
            .distinct()
            .map(p -> Pair.of(SerializationUtils.serializeBinaryRow(p.getLeft()), p.getRight()))
        ...
        // TODO-2 : 启动Spark任务，将桶数据单独压缩, 返回压缩前后的文件变更记录
        JavaRDD<byte[]> commitMessageJavaRDD = javaSparkContext.parallelize(partitionBuckets)   // 将分区 bucket 数据并行
            .mapPartitions((FlatMapFunction<Iterator<Pair<byte[], Integer>>, byte[]>) pairIterator -> {
                ...
                try {
                    while (pairIterator.hasNext()) {
                        // 压缩对应的分区桶数据
                        Pair<byte[], Integer> pair = pairIterator.next();
                        write.compact(SerializationUtils.deserializeBinaryRow(pair.getLeft()), pair.getRight(), true);
                    }

                    // 获取到当前 compaction 操作的 commit 数据， 主要包含写入前后的文件变更记录
    ...
    private void compactUnAwareBucketTable(FileStoreTable table, @Nullable Predicate filter, JavaSparkContext javaSparkContext) {
        ...
        JavaRDD<byte[]> commitMessageJavaRDD = javaSparkContext
            .parallelize(serializedTasks)
            .mapPartitions((FlatMapFunction<Iterator<byte[]>, byte[]>) taskIterator -> {
                ...
                try {
                    ...
                    while (taskIterator.hasNext()) {
                        AppendOnlyCompactionTask task = ser.deserialize(ser.getVersion(), taskIterator.next());
                        messages.add(messageSer.serialize(task.doCompact(write)));
                    }
        ...
        try (TableCommitImpl commit = table.newCommit(commitUser)) {
            ...
            List<CommitMessage> messages = new ArrayList<>(serializedMessages.size());
            for (byte[] serializedMessage : serializedMessages) {
                messages.add(messageSerializerser.deserialize(serializer.getVersion(), serializedMessage));
            }
            commit.commit(messages);
        } catch (Exception e) {
            throw new RuntimeException(e);
